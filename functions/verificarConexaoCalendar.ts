...
Deno.serve(async (req) => {
  try {
    ...
    // Verificar se o usuário conectou sua conta Google
    ...
    if (auths.length === 0) {
      return Response.json({ 
        connected: false,
        needsAuth: true,
        message: 'Google Calendar não conectado. Conecte sua conta Google.'
      });
    }
    ...
    if (tokenExpiry < now && auth.refresh_token) {
      ...
      if (!refreshResponse.ok) {
        return Response.json({ 
          connected: false,
          needsAuth: true,
          message: 'Token expirado. Reconecte sua conta Google.'
        });
      }
