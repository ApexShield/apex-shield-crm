            ...
            <div className="max-h-64 overflow-y-auto space-y-1">
              {(() => {
                const filtered = search.trim()
                  ? comissoesVenda.filter(c => (c.cliente_nome || "").toLowerCase().includes(search.toLowerCase()) || (c.produto || "").toLowerCase().includes(search.toLowerCase()))
                  : comissoesVenda;
                if (filtered.length === 0) return <p className="text-white/50 text-sm text-center py-4">Nenhuma comissão de venda ativa encontrada</p>;
