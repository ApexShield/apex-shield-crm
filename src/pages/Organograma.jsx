      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="flex items-center justify-between mb-4 md:mb-8 gap-3">
          <div className="flex items-center gap-2.5 md:gap-4">
            <div className="w-9 h-9 md:w-14 md:h-14 bg-gradient-to-br from-emerald-400 to-green-600 rounded-lg md:rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/50">
              <Network className="w-4 h-4 md:w-7 md:h-7 text-white" />
            </div>
            <div>
              <h1 className="text-lg md:text-3xl font-bold text-white">Organograma</h1>
              <p className="text-[11px] md:text-sm text-indigo-200">Hierarquia organizacional</p>
            </div>
          </div>

          {podeConvidar && (
            <Button onClick={() => setShowConviteDialog(true)} size="sm" className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 font-bold shadow-lg text-xs md:text-sm h-8 md:h-9 px-2.5 md:px-4">
              <UserPlus className="w-3.5 h-3.5 md:w-5 md:h-5 mr-1 md:mr-2" />
        ...
        <div className="flex gap-2 md:gap-4 mb-4 md:mb-6 flex-wrap">
          {["Líder de Agência", "Líder de Unidade", "Corretor"].map(tipo => {
            ...
            const Icon = cfg.icon;
            return (
              <div key={tipo} className={`flex items-center gap-1.5 ${cfg.legendBg} border px-2 py-1 md:px-3 md:py-1.5 rounded-md md:rounded-lg`}>
                <Icon className={`w-3 h-3 md:w-4 md:h-4 ${cfg.legendIcon}`} />
                <span className={`text-[10px] md:text-xs ${cfg.legendText} font-medium`}>{tipo}</span>
              </div>
            );
          })}
        </div>
